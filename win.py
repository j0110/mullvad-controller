...
def load_tunnel():
    ...
    shutil.move("mullvad.conf", "C:\\Program Files\\WireGuard\\Data\\Configurations\\mullvad.conf")
    subprocess.run(["wireguard", "/installtunnelservice", "C:\\Program Files\\WireGuard\\Data\\Configurations\\mullvad.conf"])
    subprocess.run(["sc", "config", "WireguardTunnel$mullvad", "start=auto"])
    subprocess.run(["sc", "failure", "WireguardTunnel$mullvad", "reset=0", "actions=restart/0/restart/0/restart/0"])
